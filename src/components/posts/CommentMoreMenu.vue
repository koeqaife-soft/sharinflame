...
const options = computed(() => [
  {
    key: "go_to_post",
    icon: "article",
    visible: props.showGoTo,
    type: "item"
  },
  {
    key: "jump_to_replied",
    icon: "reply",
    visible: props.comment.parent_comment_id && props.showGoTo,
    type: "item"
  },
  {
    key: props.comment.user_id == profileStore.profile?.user_id ? "delete" : "mod_delete",
    icon: "delete_forever",
    visible:
      props.comment.user_id == profileStore.profile?.user_id ||
      profileStore.profile?.permissions?.some((v) => v == "MODERATE_COMMENTS"),
    type: "item"
  },
  {
    key: "report",
